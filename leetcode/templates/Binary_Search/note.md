...
## mid 的写法
```java
int mid = begin + (end - begin) / 2;  // 防溢出，默认下取整
int mid = begin + (end - begin + 1) / 2;  // 上取整，当 end = mid - 1 时需要
```
**什么时候需要上取整**：当写了 `end = mid - 1` 而 `begin = mid` 时，下取整会死循环，需要上取整。
...
- 数组有序 / 具有单调性 → 二分
- 求满足条件的最小值/最大值 → 二分答案
- 搜索空间可以折半排除 → 二分（注意，搜索空间二分这一点容易忘记）

## 理解
- 二分的本质不是“在有序数组里找数”，而是：
  - 在一个有单调性的判定空间里，找边界 / 找答案。
- 每次二分时考虑：
  - 搜索空间是什么？
  - 单调性是什么？
  - 要找的是哪个边界？
